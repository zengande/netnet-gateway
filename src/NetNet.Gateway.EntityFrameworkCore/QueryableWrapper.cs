...
public class QueryableWrapper<TEntity> : IQueryableWrapper<TEntity> where TEntity : class
{
    private IQueryable<TEntity> _queryable;

    public QueryableWrapper(IQueryable<TEntity> queryable)
    {
...
public class DefaultQueryableWrapperFactory : IQueryableWrapperFactory, ISingletonDependency
{
    public IQueryableWrapper<TEntity> CreateWrapper<TEntity>(IQueryable<TEntity> queryable) where TEntity : class
    {
        return new QueryableWrapper<TEntity>(queryable);
    }

    public IQueryableWrapper<TEntity> CreateWrapper<TEntity, TKey>(IRepository<TEntity, TKey> repository) where TEntity : class, IEntity<TKey>
    {
#pragma warning disable CS0618
        var queryable = repository.GetDbSet().AsQueryable();
